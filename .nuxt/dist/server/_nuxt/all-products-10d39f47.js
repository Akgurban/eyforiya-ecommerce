...
import { _ as _sfc_main$2 } from "./base-product-09b837b4.js";
  ...
  async setup(__props) {
    ...
    const refetch = async () => {
      const { data: all } = await useMyFetch(
        `/api/v1/client/products/all-products?limit=15&offset=${count.value - 1}&lang=${locale.value}`
      );
      console.log(all);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`</div><div class="lead flex gap-5"><div class="md:relative absolute w-[50%] 2xl:w-[25%] lg:w-[28%]"><div class="md:block hidden shadow-2xl top-0 z-30 h-screen px-2 py-2 pb-10 bg-slate-100 rounded-md p-4"><div class="mt-5"><div class="md:text-2xl text-xl uppercase text-[#44A4DB] font-alatsi mb-2">${ssrInterpolate(_ctx.$t("category_select"))}</div><div class="ml-5"><!--[-->`);
      ssrRenderList(unref(categories).data, (item) => {
        _push(`<p class="font-alatsi hover:text-[#44A4DB] cursor-auto md:cursor-pointer">${ssrInterpolate(item == null ? void 0 : item.name)}</p>`);
      });
      _push(`<!--]--></div></div></div>`);
      if (unref(showFilter)) {
        _push(`<div class="md:hidden block fixed w-[50%] 2xl:w-[25%] lg:w-[28%] shadow-2xl top-0 z-40 h-screen px-2 py-2 pb-10 bg-slate-100 rounded-md p-4"><div class="mt-5"><div class="md:text-2xl text-xl w-min uppercase text-[#44A4DB] font-alatsi mb-2">${ssrInterpolate(_ctx.$t("category_select"))}</div><div class="ml-5"><!--[-->`);
        ssrRenderList(unref(categories).data, (item) => {
          _push(`<p class="font-alatsi hover:text-[#44A4DB] cursor-auto md:cursor-pointer">${ssrInterpolate(item == null ? void 0 : item.name)}</p>`);
        });
        _push(`<!--]--></div></div></div>`);
      ...
      _push(`</div><div class="w-full"><div style="${ssrRenderStyle({ "padding-bottom": "20px !important" })}" class="flex w-full flex-wrap gap-3 mx-auto justify-center mt-5"><!--[-->`);
...
//# sourceMappingURL=all-products-10d39f47.js.map
