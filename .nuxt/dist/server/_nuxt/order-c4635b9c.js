...
import { ref, withAsyncContext, mergeProps, withCtx, createTextVNode, unref, useSSRContext } from "vue";
...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_IconChevronRight, { class: "h-3" }, null, _parent));
      _push(`<p class="cursor-pointer">${ssrInterpolate(_ctx.$t("order"))}</p></div><div class="flex md:flex-row flex-col gap-4 mt-3"><div class="">`);
      _push(ssrRenderComponent(_component_profil_sidebar, null, null, _parent));
      _push(`</div><div class="md:w-[900px] w-full"><div class="overflow-auto"><ul><!--[-->`);
      ssrRenderList(unref(orders), (item) => {
        _push(`<div class="bg-gray-200 cursor-pointer text-lg font-bold rounded-md p-2 m-1"><div class="bg-gray-200 cursor-pointer flex justify-between items-center text-lg font-bold rounded-md p-2 m-1"><p>name: ${ssrInterpolate(item.username)}</p><p>+993 ${ssrInterpolate(item.string)}</p></div><ul class="pl-0 md:pl-10"><!--[-->`);
        ssrRenderList(item.products, (small) => {
          _push(`<li class="mt-2 odd:bg-gray-50 even:bg-gray-100 cursor-pointer flex justify-between items-center text-lg font-bold rounded-md p-2 m-1"><img${ssrRenderAttr("src", `http://duypbaha.com.tm/api/v1/uploads/images/${small == null ? void 0 : small.img_path}`)} alt="" class="h-10 w-15"><p>${ssrInterpolate(small.product_name)}</p><p>count: ${ssrInterpolate(small.count)}</p></li>`);
        });
        _push(`<!--]--></ul></div>`);
      });
...
//# sourceMappingURL=order-c4635b9c.js.map
