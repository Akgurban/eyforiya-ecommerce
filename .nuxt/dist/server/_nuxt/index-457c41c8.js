...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    try {
      const { data } = ([__temp, __restore] = withAsyncContext(() => userStore.getBrand()), __temp = await __temp, __restore(), __temp);
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div class="rounded-md p-2 m-1 flex justify-between"><p class="font-bold">Brendin Ady</p>`);
      ...
      ssrRenderList(unref(categories), (item) => {
        _push(`<div class="bg-gray-200 cursor-pointer flex justify-between items-center text-lg font-bold rounded-md p-2 m-1"><img${ssrRenderAttr("src", `http://duypbaha.com.tm/api/v1/uploads/brands/${item == null ? void 0 : item.img_path}`)} alt="" class="h-10 w-15"><p>${ssrInterpolate(item.name)}</p><div class="flex gap-3">`);
        _push(ssrRenderComponent(_component_BaseButton, {
          onClick: ($event) => unref(router).push(`/admin/brands/edit/${item.uuid}`),
          type: "primary"
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
...
//# sourceMappingURL=index-457c41c8.js.map
